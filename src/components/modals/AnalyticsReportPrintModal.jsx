...
export const AnalyticsReportPrintModal = ({
  ...
  const getPeriodTitle = () => {
    ...
    if (dateRangePreset === 'CUSTOM' && startDate && endDate) return `Custom Period Sales Report (${startDate} to ${endDate})`;
        ...
        >
                ...
                {invoices.length > 0 ? (
                  invoices.map((inv, idx) => (
                    <tr key={idx} style={{ borderBottom: '1px solid #CBD5E1', pageBreakInside: 'avoid' }}>
                      ...
                      <td style={{ padding: '0.45rem 0.3rem' }}>{inv.date}</td>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderTop: '2px solid #000000', paddingTop: '0.85rem', marginTop: '1.5rem', fontSize: '0.775rem', lineHeight: '1.5', pageBreakInside: 'avoid' }}>
            <div>
              ...
              <div>Report Generated On: <strong>{new Date().toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</strong></div>
            </div>
