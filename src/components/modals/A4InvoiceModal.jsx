...
export const A4InvoiceModal = ({ sale, onClose }) => {
  ...
  const [includeDrugActWarranty, setIncludeDrugActWarranty] = useState(
    sale.includeDrugActWarranty !== undefined ? sale.includeDrugActWarranty : true
  );
  const [includeDrapWarranty, setIncludeDrapWarranty] = useState(
    sale.includeDrapWarranty !== undefined ? sale.includeDrapWarranty : true
  );
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.15fr 1.45fr 1.2fr', gap: '0.95rem', border: '2px solid #000000', padding: '0.85rem 1rem', marginBottom: '1.85rem', fontSize: '0.8rem', lineHeight: '1.7' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
              ...
              <div><strong>Sale Order #:</strong> <span style={{ fontFamily: 'monospace' }}>{sale.saleOrderNo || '0170804'}</span></div>
              <div><strong>DSS Id:</strong> <span style={{ fontFamily: 'monospace' }}>{sale.dssId || '3032285'}</span></div>
              <div><strong>Reference No:</strong> {sale.referenceNo || 'Naeem Shah'}</div>
              <div><strong>Booking Man:</strong> {sale.bookingMan || 'Naeem Shah'}</div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem', borderLeft: '2px solid #000000', paddingLeft: '0.85rem' }}>
              ...
              <div style={{ marginTop: '0.2rem' }}>
                <strong>Cust. FBR Status:</strong> <span style={{ border: '1.5px solid #000000', padding: '0.1rem 0.45rem', fontWeight: 'bold' }}>{sale.fbrStatus || 'FILER As Per FBR On 03-11-2025'}</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ borderTop: '2.5px solid #000000', borderBottom: '2.5px solid #000000', padding: '0.75rem 0.85rem', marginBottom: '1.85rem', fontSize: '0.875rem', lineHeight: '1.6', pageBreakInside: 'avoid' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem', fontWeight: 'bold' }}>
              <div>
                <span>Gross: Rs. {Number(sale.grossSubtotal || sale.subtotal || 600).toFixed(2)}</span> &nbsp;|&nbsp;
                <span>{getTaxConfig().saleTaxName || 'Sale Tax 18%'}: Rs. {Number(sale.totalSaleTax || 108).toFixed(2)}</span> &nbsp;|&nbsp;
                <span>{getTaxConfig().adTaxName || 'AdTax 4%'}: Rs. {Number(sale.totalAdTax || 24).toFixed(2)}</span> &nbsp;|&nbsp;
                <span>{getTaxConfig().advTaxName || 'Adv Tax 0.5%'}: Rs. {Number(sale.totalAdvTax || 3.00).toFixed(2)}</span>
              </div>
              <div style={{ fontSize: '1.1rem', textDecoration: 'underline', letterSpacing: '0.02em' }}>
                NET AMOUNT: Rs. {Number(sale.netTotal || 735.00).toFixed(2)}
              </div>
            </div>

            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1.75rem', marginTop: '0.45rem', fontSize: '0.825rem' }}>
              <span>Cash Received: <strong>Rs. {Number(sale.tendered || sale.netTotal || 800.00).toFixed(2)}</strong></span>
              <span>Change / Balance: <strong>Rs. {Number(sale.change || 0.00).toFixed(2)}</strong></span>
            </div>
          </div>
